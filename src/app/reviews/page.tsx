...
export default function ReviewsPage() {
  ...
  // ===== Firestore query (client-side only, sorted) =====
  const reviewsQuery = useMemoFirebase(() => {
    if (!firestore) return null;
    let q: Query<DocumentData> = collection(firestore, 'reviews');

    if (sort === 'rating_desc') q = query(q, orderBy('rating', 'desc'));
    else if (sort === 'upvotes_desc') q = query(q, orderBy('upvotes', 'desc'));
  ...
  const filteredReviews = useMemo(() => {
    ...
    return reviewsData.filter((review) => {
      ...
      const genreMatch = genre === 'all' || (review.genre?.toLowerCase() === genre.toLowerCase());
  ...
  const genres = useMemo(
    () => (reviewsData ? [...new Set(reviewsData.map((r) => r.genre).filter(Boolean).sort())] : []),
    [reviewsData]
  );
  ...
  const years = useMemo(
    () =>
      reviewsData
        ? [...new Set(reviewsData.map((r) => r.year.toString()))].sort((a, b) => parseInt(b) - parseInt(a))
        : [],
    [reviewsData]
  );
